Public Class frm_eliminar3
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        If clase.dt.Tables("tabla11").Rows.Count > 0 Then
            ...
            clase.consultar("SELECT dt_cantidad FROM dettransferencia WHERE (dt_trnumero = " & frm_transferencias_desde_bodega.TextBox1.Text & " AND dt_codarticulo =" & codigo_articulo & ")", "tbl")
            If clase.dt.Tables("tbl").Rows.Count > 0 Then
                ...
                If v = 6 Then
                    ...
                    If totcant > 1 Then
                        clase.actualizar("UPDATE dettransferencia SET dt_cantidad = " & totcant - 1 & " WHERE (dt_trnumero = " & frm_transferencias_desde_bodega.TextBox1.Text & " AND dt_codarticulo =" & codigo_articulo & ")")
                    Else
                        clase.borradoautomatico("Delete from dettransferencia WHERE (dt_trnumero = " & frm_transferencias_desde_bodega.TextBox1.Text & " AND dt_codarticulo =" & codigo_articulo & ")")
                    End If
                    frm_transferencias_desde_bodega.EstablecerFoto(codigo_articulo)
                    frm_transferencias_desde_bodega.llenar_grilla(frm_transferencias_desde_bodega.TextBox1.Text)
                    Dim x As Short
                    For x = 0 To frm_transferencias_desde_bodega.DataGridView1.RowCount - 1
                        If codigo_articulo = frm_transferencias_desde_bodega.DataGridView1.Item(0, x).Value Then
                            frm_transferencias_desde_bodega.DataGridView1.CurrentCell = frm_transferencias_desde_bodega.DataGridView1.Item(0, x)
                            Exit For
                        End If
                    Next
                    TextBox4.Text = ""
