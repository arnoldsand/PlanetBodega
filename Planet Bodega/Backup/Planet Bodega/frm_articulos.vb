            ...
            Case "Codigo"
                If txt <> "" Then
                    If Len(textBox2.Text) = 13 Then
                        complebus = consulta & " WHERE (articulos.ar_codigobarras = '" & txt & "')" & agrupamiento
                    Else
                        complebus = consulta & " WHERE (articulos.ar_codigo = " & txt & ")" & agrupamiento
                    End If
                Else
                    complebus = consulta
                End If
            Case "Descripción"
                complebus = consulta & " WHERE articulos.ar_descripcion like '" & txt & "%'" & agrupamiento & " ORDER BY articulos.ar_descripcion asc"
            Case "Referencia"
                complebus = consulta & " WHERE articulos.ar_referencia like '" & txt & "%'" & agrupamiento & " ORDER BY articulos.ar_referencia asc"
            Case ""
                complebus = consulta & agrupamiento & " ORDER BY articulos.ar_fechaingreso DESC"
        End Select
